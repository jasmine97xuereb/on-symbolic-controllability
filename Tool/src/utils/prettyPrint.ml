...
let rec pretty_print_verdict_list (w: Ast.Monitor.Verdict.t list): string = 
  match w with 
  | [] -> ""
  | w::ws ->
    (match w.verd with 
    | ZERO -> pretty_print_verdict_list ws
    | ONE -> "1 " ^ pretty_print_verdict_list ws
    | TWO -> "2 " ^ pretty_print_verdict_list ws
    | _ -> "")
