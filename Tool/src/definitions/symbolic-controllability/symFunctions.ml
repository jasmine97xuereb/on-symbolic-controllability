...
let rec osaft (cm: (Ast.Expression.t list * Ast.Monitor.t list)) (evt: Ast.SymbolicEvent.t) (c: Ast.Expression.t) (relation: (Ast.Expression.t list * Ast.Monitor.t list) list) (unseen_cms: (Ast.Expression.t list * Ast.Monitor.t list) Queue.t) =
  ...
  in let rec saft_aux (m: Ast.Monitor.t list) (res: (Ast.Expression.t list * Ast.Monitor.t list)) (saft_cms: (Ast.Expression.t list * Ast.Monitor.t list) list) =
    (match m with
    | [] -> 
...
let rec get_next_unseen (relation: (Ast.Expression.t list * Ast.Monitor.t list) list) (unseen_cms: (Ast.Expression.t list * Ast.Monitor.t list) Queue.t): (Ast.Expression.t list * Ast.Monitor.t list) = 
  ...
  then ([],[]) 
    ...
    let next_m = Queue.pop unseen_cms in
      ...
      then (
        ...
        get_next_unseen relation unseen_cms
      )
      else (
