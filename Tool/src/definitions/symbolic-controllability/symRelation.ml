...
let rec create_all_combinations (b: Ast.Expression.t list) (indices_list: int list list) (cs: Ast.Expression.t list) (to_add: Ast.Expression.t list list): Ast.Expression.t list = 
  ...
  | i::is -> 
    ...
    in (
      match comb with 
      | [] -> (create_all_combinations b is cs to_add)
      | _ -> comb  @ (create_all_combinations b is cs to_add)
    )
...
let rec sc (b: Ast.Expression.t list) (cs: Ast.Expression.t list) (result: Ast.Expression.t list list): Ast.Expression.t list =
  let partition = List.partition check_contains_expression_tree cs in 
  print_all_messages("First partition is " ^ pretty_print_evt_list (fst partition));
  ...
  let t_list = List.map (fun x -> traverse_expression_tree x []) (fst partition) 
  in print_all_messages("traversed tree is "); 
  List.map (fun x -> print_all_messages (pretty_print_evt_list x)) t_list;  
  ...
  let filtered_t = filter traversed_list in  
  let exp_tree_comb = List.map (fun x -> [x]) (filtered_t) in
  ...
  in let rec combinations (n:int) (to_add: Ast.Expression.t list list) = 
    ...
    | n -> (create_all_combinations b (combnk n num_list) (snd partition) to_add) @ (combinations (n-1) to_add)

  in combinations (List.length (snd partition)) exp_tree_comb 
