...
(* convert a goal expr to ast exp *)
...
(* function to convert a list of goals into an of expressions of the form Ast.Expression.t *)
(* if there is one goal, return a list of expressions added together via conjunctions) *)
(* if there is more than one goal, exps are added via disjunctions (since mutiple goals are added together via disjunctions) *)
let rec goals_to_exp (goals: Z3.Goal.goal list): Ast.Expression.t = 
  match goals with 
  | [] -> Ast.Expression.Literal(Bool(true))
  | g::[] -> z3_to_exp [Goal.as_expr g]
  | g::gs -> add_binary_condition (z3_to_exp [Goal.as_expr g]) (goals_to_exp gs) Ast.Expression.BinaryExp.Or
    ...
    let cndts = exp_list_to_z3 c [] ctx in
      ...
      let result = (Tactic.apply (and_then ctx (mk_tactic ctx ("simplify")) (repeat ctx (or_else ctx (mk_tactic ctx "split-clause") (mk_tactic ctx "skip")) 100) []) g None) in
        ...
        let subgoals = get_subgoals result 
          ...
          | [] -> []
          | g::[] -> [z3_to_exp [Goal.as_expr g]]
          | g::gs -> [z3_to_exp [Goal.as_expr g]] @ (goals_to_dnf gs)  
