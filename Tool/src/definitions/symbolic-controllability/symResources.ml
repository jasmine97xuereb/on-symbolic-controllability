...
(* function to iterate through a list of boolean conditions and split b1 ^ b2 into b1 and b2 *)
let rec split_or (b: Ast.Expression.t list): Ast.Expression.t list = 
  let rec inner_split (b: Ast.Expression.t): Ast.Expression.t list = 
    match b with 
    | Ast.Expression.BinaryExp(x) -> ( 
      match x.operator with
      | Or -> (inner_split x.arg_lt) @ (inner_split x.arg_rt)
      | _ -> [b] )
    | _ -> [b]
  in match b with
  | [] -> [] 
  | b::[] -> inner_split b
  | b::bs -> (inner_split b) @ (split_or bs) 
...
let rec uch (var_uch: Ast.Identifier.t) (b: Ast.Expression.t): bool = 
  ...
  and fetch_x (var_uch: Ast.Identifier.t) (b: Ast.Expression.t list) (rem: Ast.Expression.t list): (Ast.Expression.t list * Ast.Expression.t list) = 
  ...
  and is_literal (e: Ast.Expression.t) = 
    match e with
      ...
      | _ -> 
        (
          ...
          then false 
            ...
            let free = fv ((fst result), []) Vars.empty
            in 
